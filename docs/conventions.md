...
- Filename: `<name>.md` (kebab-case)
- Location: `skills/<name>.md` (source of truth)
...
### Multi-Model Skill Resolution

The canonical skill source is always `skills/<name>.md`. Model-specific adaptations are **derived**, not primary. Each agent runtime resolves skills through its own directory:

| Runtime | Resolution path | Format | Generated by |
|---------|----------------|--------|-------------|
